...
object AssociationList {
  ...
  def insert[Key, Value](key: Key, value: Value,
                         alist: AList[Key, Value]): AList[Key, Value] =
    alist match {
  ...
  def lookup[Key, Value](key: Key,
                         alist: AList[Key, Value]): Option[Value] =
    alist match {
      case Nil => None
      case (k, v) :: tail =>
        if (k == key) {
