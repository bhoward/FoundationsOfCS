...
class MergeSortImmutableListNaiveTest extends TestSpec {
  import MergeSortImmutableListNaive.mergeSort
  ...
  it should "overflow when sorting a large sorted list" in {
    val xs = List.range(1, 100000)
    a [StackOverflowError] should be thrownBy mergeSort(xs)
  }
}